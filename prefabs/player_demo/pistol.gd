...
func _shoot():
  ...
  #$FireRay.translation = camera.translation
  #$FireRay.cast_to = to
  var ray = self.camera.get_node("FireRay")
  if(ray.is_colliding()):
    if(ray.get_collider().get_owner().is_in_group("enemy")):
      #Quick n dirty
      ray.get_collider().get_owner().hit(ray.get_collider(), ray.get_collision_point(), 25)
